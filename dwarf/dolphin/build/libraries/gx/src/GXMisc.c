/*
    Compile unit: c:\dolphin\build\libraries\gx\src\GXMisc.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x800222E0 -> 0x800227BC
*/
static void (* TokenCB)(unsigned short); // size: 0x4, address: 0x8013B1B0
static void (* DrawDoneCB)(); // size: 0x4, address: 0x8013B1B4
static unsigned char DrawDone; // size: 0x1, address: 0x8013B1B8
// total size: 0x8
struct OSThreadLink {
    // Members
...
static struct OSThreadQueue FinishQueue; // size: 0x8, address: 0x8013B1BC
// total size: 0x10
struct _GXTexRegion {
    // Members
...
// Range: 0x80022348 -> 0x80022408
void GXSetDrawSync(unsigned short token /* r31 */) {
    // Local variables
    int enabled; // r31
...
// Range: 0x800224AC -> 0x800224C0
void GXPokeAlphaMode(enum _GXCompare func /* r1+0x0 */, unsigned char threshold /* r1+0x4 */) {
    // References
    // -> void * __peReg;
}
...
// Range: 0x800224C0 -> 0x800224D4
void GXPokeAlphaRead(enum _GXAlphaReadMode mode /* r1+0x0 */) {
    // Local variables
...
// Range: 0x800224D4 -> 0x800224F0
void GXPokeAlphaUpdate(unsigned char update_enable /* r1+0x0 */) {
    // Local variables
    unsigned long reg; // r3
...
// Range: 0x800224F0 -> 0x80022578
void GXPokeBlendMode(enum _GXBlendMode type /* r1+0x0 */, enum _GXBlendFactor src_factor /* r1+0x4 */, enum _GXBlendFactor dst_factor /* r1+0x8 */, enum _GXLogicOp op /* r1+0xC */) {
    // Local variables
    unsigned long reg; // r7

    // References
    // -> void * __peReg;
}

// Range: 0x80022578 -> 0x80022594
void GXPokeColorUpdate(unsigned char update_enable /* r1+0x0 */) {
    // Local variables
    unsigned long reg; // r3
...
// Range: 0x80022594 -> 0x800225A8
void GXPokeDstAlpha(unsigned char enable /* r1+0x0 */, unsigned char alpha /* r1+0x1 */) {
    // Local variables
    unsigned long reg; // r1+0x0
...
// Range: 0x800225A8 -> 0x800225C4
void GXPokeDither(unsigned char dither /* r1+0x0 */) {
    // Local variables
    unsigned long reg; // r3
...
// Range: 0x800225C4 -> 0x800225EC
void GXPokeZMode(unsigned char compare_enable /* r1+0x0 */, enum _GXCompare func /* r1+0x4 */, unsigned char update_enable /* r1+0x8 */) {
    // Local variables
    unsigned long reg; // r1+0x0
...
// Range: 0x800225EC -> 0x80022630
void (* GXSetDrawSyncCallback(void (* cb)(unsigned short) /* r30 */))(unsigned short) {
    // Local variables
...
// Range: 0x80022630 -> 0x800226B8
static void GXTokenInterruptHandler(struct OSContext * context /* r30 */) {
    // Local variables
...
// Range: 0x800226B8 -> 0x8002273C
static void GXFinishInterruptHandler(struct OSContext * context /* r31 */) {
    // Local variables
    struct OSContext exceptionContext; // r1+0x10
    unsigned long reg; // r1+0x8
