/*
    Compile unit: D:\Pure3d\development\lib\zlib\infcodes.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x80093E3C -> 0x80094844
*/
enum /* @enum$2infcodes_c */ {
    START = 0,
    LEN = 1,
    LENEXT = 2,
    DIST = 3,
...
struct inflate_codes_state {
    ...
    union {
        ...
        } copy; // offset 0x0, size 0x8
    } sub; // offset 0x8, size 0x8
...
// Range: 0x80093E3C -> 0x80093EC0
struct inflate_codes_state * inflate_codes_new(unsigned int bl /* r28 */, unsigned int bd /* r29 */, struct inflate_huft_s * tl /* r30 */, struct inflate_huft_s * td /* r31 */, struct z_stream_s * z /* r1+0x18 */) {
    // Local variables
    struct inflate_codes_state * c; // r3
}

unsigned int inflate_mask[17]; // size: 0x44, address: 0x8010C6B0
enum /* @enum$1infcodes_c */ {
    TYPE = 0,
    LENS = 1,
    STORED = 2,
    TABLE = 3,
...
// Range: 0x80093EC0 -> 0x80094810
int inflate_codes(struct inflate_blocks_state * s /* r29 */, struct z_stream_s * z /* r30 */, int r /* r5 */) {
    // Local variables
    unsigned int j; // r7
    struct inflate_huft_s * t; // r7
    unsigned int e; // r1+0x8
    unsigned long b; // r26
    unsigned int k; // r25
    unsigned char * p; // r24
    unsigned int n; // r23
    ...
    unsigned int m; // r3
    unsigned char * f; // r22
    struct inflate_codes_state * c; // r31

    // References
    // -> unsigned int inflate_mask[17];
}

// Range: 0x80094810 -> 0x80094844
void inflate_codes_free(struct inflate_codes_state * c /* r1+0x8 */, struct z_stream_s * z /* r4 */) {}
