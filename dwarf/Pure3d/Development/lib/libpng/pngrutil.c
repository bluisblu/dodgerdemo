/*
    Compile unit: D:\Pure3d\development\lib\libpng\pngrutil.c
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8008D114 -> 0x80090F24
*/
// Range: 0x8008D114 -> 0x8008D140
unsigned long png_get_uint_32(unsigned char * buf /* r1+0x0 */) {}
...
// Range: 0x8008D140 -> 0x8008D194
void png_crc_read(struct png_struct_def * png_ptr /* r29 */, unsigned char * buf /* r30 */, unsigned long length /* r31 */) {}

// Range: 0x8008D194 -> 0x8008D294
int png_crc_finish(struct png_struct_def * png_ptr /* r27 */, unsigned long skip /* r4 */) {
    // Local variables
    unsigned long i; // r29
    unsigned long istop; // r28
}

// Range: 0x8008D294 -> 0x8008D354
int png_crc_error(struct png_struct_def * png_ptr /* r30 */) {
    // Local variables
    unsigned char crc_bytes[4]; // r1+0x1C
    int need_crc; // r31
}

// Range: 0x8008D354 -> 0x8008D620
void png_handle_IHDR(struct png_struct_def * png_ptr /* r22 */, struct png_info_struct * info_ptr /* r23 */, unsigned long length /* r24 */) {
    // Local variables
    unsigned char buf[13]; // r1+0x2C
    unsigned long width; // r30
    unsigned long height; // r29
    int bit_depth; // r28
    int color_type; // r27
    int compression_type; // r26
    int filter_type; // r25
    int interlace_type; // r24
}

// Range: 0x8008D620 -> 0x8008D7FC
void png_handle_PLTE(struct png_struct_def * png_ptr /* r28 */, struct png_info_struct * info_ptr /* r29 */, unsigned long length /* r30 */) {
    // Local variables
    struct png_color_struct * palette; // r27
    int num; // r26
    int i; // r25
    unsigned char buf[3]; // r1+0x14
}

// Range: 0x8008D7FC -> 0x8008D898
void png_handle_IEND(struct png_struct_def * png_ptr /* r29 */, struct png_info_struct * info_ptr /* r30 */, unsigned long length /* r31 */) {}
...
// Range: 0x8008D898 -> 0x8008DA70
void png_handle_gAMA(struct png_struct_def * png_ptr /* r29 */, struct png_info_struct * info_ptr /* r30 */, unsigned long length /* r28 */) {
    // Local variables
    unsigned long igamma; // r28
    float file_gamma; // f1
    unsigned char buf[4]; // r1+0x1C

    // References
    // -> struct _FILE __files[];
}

// Range: 0x8008DA70 -> 0x8008DC1C
void png_handle_sBIT(struct png_struct_def * png_ptr /* r30 */, struct png_info_struct * info_ptr /* r31 */, unsigned long length /* r27 */) {
    // Local variables
    unsigned long truelen; // r28
    unsigned char buf[4]; // r1+0x14
}

// Range: 0x8008DC1C -> 0x8008E3B0
void png_handle_cHRM(struct png_struct_def * png_ptr /* r28 */, struct png_info_struct * info_ptr /* r29 */, unsigned long length /* r31 */) {
    // Local variables
    unsigned char buf[4]; // r1+0xD4
    float white_x; // f31
    float white_y; // f30
    float red_x; // f29
    float red_y; // f28
...
// Range: 0x8008E3B0 -> 0x8008E6C0
void png_handle_sRGB(struct png_struct_def * png_ptr /* r28 */, struct png_info_struct * info_ptr /* r29 */, unsigned long length /* r30 */) {
    // Local variables
    int intent; // r30
    unsigned char buf[1]; // r1+0x74

    // References
    // -> struct _FILE __files[];
}

// Range: 0x8008E6C0 -> 0x8008E984
void png_handle_tRNS(struct png_struct_def * png_ptr /* r30 */, struct png_info_struct * info_ptr /* r31 */, unsigned long length /* r28 */) {
    // Local variables
    unsigned char buf[6]; // r1+0x34
    unsigned char buf[6]; // r1+0x2C
}

// Range: 0x8008E984 -> 0x8008EBB0
void png_handle_bKGD(struct png_struct_def * png_ptr /* r30 */, struct png_info_struct * info_ptr /* r31 */, unsigned long length /* r27 */) {
    // Local variables
    unsigned long truelen; // r28
    unsigned char buf[6]; // r1+0x2C
}

// Range: 0x8008EBB0 -> 0x8008ED44
void png_handle_hIST(struct png_struct_def * png_ptr /* r30 */, struct png_info_struct * info_ptr /* r31 */, unsigned long length /* r27 */) {
    // Local variables
    int num; // r28
    int i; // r27
    unsigned char buf[2]; // r1+0x1C
}

// Range: 0x8008ED44 -> 0x8008EEBC
void png_handle_pHYs(struct png_struct_def * png_ptr /* r28 */, struct png_info_struct * info_ptr /* r29 */, unsigned long length /* r30 */) {
    // Local variables
    unsigned char buf[9]; // r1+0x2C
    unsigned long res_x; // r5
    unsigned long res_y; // r6
    int unit_type; // r7
}

// Range: 0x8008EEBC -> 0x8008F034
void png_handle_oFFs(struct png_struct_def * png_ptr /* r28 */, struct png_info_struct * info_ptr /* r29 */, unsigned long length /* r30 */) {
    // Local variables
    unsigned char buf[9]; // r1+0x2C
    unsigned long offset_x; // r5
    unsigned long offset_y; // r6
    int unit_type; // r7
}

// Range: 0x8008F034 -> 0x8008F318
void png_handle_pCAL(struct png_struct_def * png_ptr /* r21 */, struct png_info_struct * info_ptr /* r22 */, unsigned long length /* r20 */) {
    // Local variables
    char * purpose; // r27
    long X0; // r26
    long X1; // r25
    unsigned char type; // r1+0x10
    unsigned char nparams; // r24
    char * buf; // r9
    char * units; // r23
    char * endptr; // r1+0x10
    char * * params; // r20
    unsigned long slength; // r20
    int i; // r1+0x10
}
...
// Range: 0x8008F318 -> 0x8008F46C
void png_handle_tIME(struct png_struct_def * png_ptr /* r28 */, struct png_info_struct * info_ptr /* r29 */, unsigned long length /* r30 */) {
    // Local variables
    unsigned char buf[7]; // r1+0x24
    struct png_time_struct mod_time; // r1+0x1C
}

// Range: 0x8008F46C -> 0x8008F590
void png_handle_tEXt(struct png_struct_def * png_ptr /* r30 */, struct png_info_struct * info_ptr /* r31 */, unsigned long length /* r27 */) {
    // Local variables
    struct png_text_struct * text_ptr; // r29
    char * key; // r28
    char * text; // r27
    unsigned long skip; // r1+0x8
    unsigned long slength; // r27
}

static char msg$825[26]; // size: 0x1A, address: 0x8010AE54
// Range: 0x8008F590 -> 0x8008F944
void png_handle_zTXt(struct png_struct_def * png_ptr /* r30 */, struct png_info_struct * info_ptr /* r31 */, unsigned long length /* r22 */) {
    // Local variables
    struct png_text_struct * text_ptr; // r22
    char * key; // r26
    char * text; // r25
    int comp_type; // r24
    unsigned long slength; // r23
    unsigned long text_size; // r1+0x8
    unsigned long key_size; // r22
    int ret; // r1+0x8
    char * tmp; // r21
    unsigned long text_size; // r1+0x8
    char umsg[50]; // r1+0x14

    // References
    // -> static char msg$825[26];
}

// Range: 0x8008F944 -> 0x8008F9D0
void png_handle_unknown(struct png_struct_def * png_ptr /* r29 */, struct png_info_struct * info_ptr /* r30 */, unsigned long length /* r31 */) {}

// Range: 0x8008F9D0 -> 0x8008FA88
void png_check_chunk_name(struct png_struct_def * png_ptr /* r3 */, unsigned char * chunk_name /* r1+0xC */) {}

// Range: 0x8008FA88 -> 0x8008FDCC
void png_combine_row(struct png_struct_def * png_ptr /* r6 */, unsigned char * row /* r4 */, int mask /* r31 */) {
    // Local variables
    unsigned char * sp; // r7
    unsigned char * dp; // r4
    int s_inc; // r8
    ...
    unsigned long row_width; // r1+0x8
    int value; // r1+0x8
    unsigned char * sp; // r7
    unsigned char * dp; // r4
    int s_start; // r8
    ...
    unsigned char * dp; // r28
    unsigned long pixel_bytes; // r27
    unsigned long i; // r26
    unsigned long row_width; // r25
    unsigned char m; // r24
}

int png_pass_inc[7]; // size: 0x1C, address: 0x80109FC8
// Range: 0x8008FDCC -> 0x800901D8
void png_do_read_interlace(struct png_row_info_struct * row_info /* r29 */, unsigned char * row /* r1+0xC */, int pass /* r1+0x10 */, unsigned long transformations /* r1+0x14 */) {
    // Local variables
    unsigned long final_width; // r30
    unsigned char * sp; // r6
    unsigned char * dp; // r8
    int sshift; // r7
    int dshift; // r9
    ...
    int jstop; // r1+0x8
    int j; // r1+0x8
    unsigned long pixel_bytes; // r28
    unsigned char * sp; // r27
    unsigned char * dp; // r26
    int jstop; // r1+0x8
    unsigned long i; // r25
    unsigned char v[8]; // r1+0x18
    int j; // r24
...
// Range: 0x800901D8 -> 0x800908D8
void png_read_filter_row(struct png_struct_def * png_ptr /* r3 */, struct png_row_info_struct * row_info /* r1+0xC */, unsigned char * row /* r31 */, unsigned char * prev_row /* r6 */, int filter /* r1+0x18 */) {
    // Local variables
    unsigned long i; // r4
    unsigned long istop; // r1+0x8
    unsigned long bpp; // r1+0x8
    unsigned char * rp; // r5
    unsigned char * lp; // r6
    unsigned long i; // r4
    unsigned long istop; // r1+0x8
...
// Range: 0x800908D8 -> 0x80090C44
void png_read_finish_row(struct png_struct_def * png_ptr /* r30 */) {
    // Local variables
    char extra; // r1+0x1C
    int ret; // r1+0x8
    unsigned char chunk_length[4]; // r1+0x18

    // References
    // -> unsigned char png_IDAT[5];
...
// Range: 0x80090C44 -> 0x80090F24
void png_read_start_row(struct png_struct_def * png_ptr /* r31 */) {
    // Local variables
    int max_pixel_depth; // r6
    unsigned long row_bytes; // r3
