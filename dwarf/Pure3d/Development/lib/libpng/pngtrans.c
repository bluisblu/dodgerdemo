/*
    Compile unit: D:\Pure3d\development\lib\libpng\pngtrans.c
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80091680 -> 0x800928AC
*/
// total size: 0xC
...
// Range: 0x80091680 -> 0x80091690
void png_set_bgr(struct png_struct_def * png_ptr /* r1+0x0 */) {}

// Range: 0x80091690 -> 0x80091700
void png_set_filler(struct png_struct_def * png_ptr /* r1+0x0 */, unsigned long filler /* r1+0x4 */, int filler_loc /* r1+0x8 */) {}

// Range: 0x80091700 -> 0x800917DC
void png_do_invert(struct png_row_info_struct * row_info /* r1+0x0 */, unsigned char * row /* r4 */) {
    // Local variables
    unsigned char * rp; // r4
    unsigned long i; // r6
...
// Range: 0x800917DC -> 0x800918BC
void png_do_swap(struct png_row_info_struct * row_info /* r1+0x0 */, unsigned char * row /* r4 */) {
    // Local variables
    unsigned char * rp; // r4
...
static unsigned char onebppswaptable[256]; // size: 0x100, address: 0x8010AFE0
static unsigned char twobppswaptable[256]; // size: 0x100, address: 0x8010B0E0
static unsigned char fourbppswaptable[256]; // size: 0x100, address: 0x8010B1E0
// Range: 0x800918BC -> 0x800919AC
void png_do_packswap(struct png_row_info_struct * row_info /* r1+0x0 */, unsigned char * row /* r4 */) {
    // Local variables
    unsigned char * rp; // r4
    unsigned char * end; // r1+0x0
    unsigned char * table; // r5

    // References
    // -> static unsigned char fourbppswaptable[256];
    // -> static unsigned char twobppswaptable[256];
    // -> static unsigned char onebppswaptable[256];
}

// Range: 0x800919AC -> 0x80092438
void png_do_strip_filler(struct png_row_info_struct * row_info /* r1+0x0 */, unsigned char * row /* r4 */, unsigned long flags /* r1+0x8 */) {
    // Local variables
    unsigned char * sp; // r4
    unsigned char * dp; // r6
...
// Range: 0x80092438 -> 0x800928AC
void png_do_bgr(struct png_row_info_struct * row_info /* r1+0x0 */, unsigned char * row /* r4 */) {
    // Local variables
    unsigned long row_width; // r3
    unsigned char * rp; // r4
    unsigned long i; // r1+0x0
    unsigned char save; // r5
    unsigned char * rp; // r4
    unsigned long i; // r1+0x0
    unsigned char save; // r5
    unsigned char * rp; // r4
