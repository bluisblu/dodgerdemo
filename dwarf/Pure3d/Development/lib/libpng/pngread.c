/*
    Compile unit: D:\Pure3d\development\lib\libpng\pngread.c
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80084004 -> 0x80084FD8
*/
// total size: 0xC
...
// Range: 0x80084004 -> 0x800841C0
struct png_struct_def * png_create_read_struct_2(const char * user_png_ver /* r29 */, void * error_ptr /* r23 */, void (* error_fn)(struct png_struct_def *, char *) /* r24 */, void (* warn_fn)(struct png_struct_def *, char *) /* r25 */, void * mem_ptr /* r26 */, void * (* malloc_fn)(struct png_struct_def *, unsigned long) /* r27 */, void (* free_fn)(struct png_struct_def *, struct png_struct_def *) /* r28 */) {
    // Local variables
    struct png_struct_def * png_ptr; // r30

    // References
    // -> char png_libpng_ver[12];
}

// Range: 0x800841C0 -> 0x800842DC
void png_read_init(struct png_struct_def * png_ptr /* r30 */) {
    // Local variables
    long * tmp_jmp[70]; // r1+0xC
}
...
// Range: 0x800842DC -> 0x80084704
void png_read_info(struct png_struct_def * png_ptr /* r27 */, struct png_info_struct * info_ptr /* r28 */) {
    // Local variables
    unsigned long num_checked; // r30
    unsigned long num_to_check; // r29
    unsigned char chunk_length[4]; // r1+0x10
    unsigned long length; // r29

    // References
    // -> unsigned char png_zTXt[5];
    // -> unsigned char png_tRNS[5];
    // -> unsigned char png_tIME[5];
    // -> unsigned char png_tEXt[5];
    // -> unsigned char png_sRGB[5];
    // -> unsigned char png_sBIT[5];
    // -> unsigned char png_pHYs[5];
    // -> unsigned char png_pCAL[5];
    // -> unsigned char png_oFFs[5];
    // -> unsigned char png_hIST[5];
    // -> unsigned char png_gAMA[5];
    // -> unsigned char png_cHRM[5];
    // -> unsigned char png_bKGD[5];
    // -> unsigned char png_IDAT[5];
    // -> unsigned char png_IEND[5];
    // -> unsigned char png_PLTE[5];
    // -> unsigned char png_IHDR[5];
}

// Range: 0x80084704 -> 0x80084758
void png_read_update_info(struct png_struct_def * png_ptr /* r30 */, struct png_info_struct * info_ptr /* r31 */) {}

int png_pass_dsp_mask[7]; // size: 0x1C, address: 0x8010A038
int png_pass_mask[7]; // size: 0x1C, address: 0x8010A01C
// Range: 0x80084758 -> 0x80084C98
void png_read_row(struct png_struct_def * png_ptr /* r28 */, unsigned char * row /* r29 */, unsigned char * dsp_row /* r30 */) {
    // Local variables
    int ret; // r1+0x8
    unsigned char chunk_length[4]; // r1+0x14
...
// Range: 0x80084C98 -> 0x80084D74
void png_destroy_read_struct(struct png_struct_def * * png_ptr_ptr /* r25 */, struct png_info_struct * * info_ptr_ptr /* r26 */, struct png_info_struct * * end_info_ptr_ptr /* r27 */) {
    // Local variables
    struct png_struct_def * png_ptr; // r31
    struct png_info_struct * info_ptr; // r30
    struct png_info_struct * end_info_ptr; // r29
    void (* free_fn)(struct png_struct_def *, struct png_struct_def *); // r28
}

// Range: 0x80084D74 -> 0x80084FD8
void png_read_destroy(struct png_struct_def * png_ptr /* r31 */, struct png_info_struct * info_ptr /* r4 */, struct png_info_struct * end_info_ptr /* r27 */) {
    // Local variables
    long * tmp_jmp[70]; // r1+0x14
    void (* error_fn)(struct png_struct_def *, char *); // r30
    void (* warning_fn)(struct png_struct_def *, char *); // r29
    void * error_ptr; // r28
    void (* free_fn)(struct png_struct_def *, struct png_struct_def *); // r27
    int i; // r27
    int istop; // r28
    int i; // r27
    int istop; // r28
